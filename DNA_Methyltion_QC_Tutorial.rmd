...
params: 
  Name: DNA Methylation QC for project 10396
  SampleSheet: NorCog_methylation_samples_for_QC.csv
  PathToProjectFolder: /mnt/data1/Morteza/Methylation_Data/10399
  Mset: NIH_msetEPIC_10399.rdat
  RGset: NIH_RGset_10399.rdat
  IntensityThreshold: 750
  #Genotype: /mnt/data1/IndiaWorkshop/EXTEND_genotype_QCd_59EPICSNPs.csv
  fam:
  SamplesPassed: SamplesPassedQC_10399.csv
  SamplesFailed: SamplesFailedQC_10399.csv
  Normalised: NIH_Normalised_10399.rdat
...
#convert Rmd file into script:
#knitr::purl("your Rmd file.Rmd")
```
...
#setwd("/mnt/data1/Morteza/Methylation_Data")
setwd(params$PathToProjectFolder)

SampleSheet<-read.csv(file = params$SampleSheet, stringsAsFactors = FALSE, sep = ",", na.strings = "")
...
idatPath<-c("/mnt/data1/Morteza/Methylation_Data/idatfiles/")
...
if(file.exists(file= params$RGset)){
  ...
  print(paste("Loaded RGset:", params$RGset))
} else{
...
#all.equal(x, y)
 ...
 #dim(RGset)
 #dim(SampleSheet)
 #remove samples based on basenames
 #removed<-as.character(read.table(file = "removed_samples.txt")$V1)
 #indexes<-RGset$Basename%in%removed
 #table(indexes)
 #RGset=RGset[,!indexes]
 #msetEPIC=msetEPIC[,!indexes]
# dim(msetEPIC)
# dim(RGset)
```
...
#geno<-read.csv(params$Genotype, row.names = 1, h=T) #The second row is where the samples are unique to individuals
...
#tissue.predictor <- tissue_predictor(betas)
#QCmetrics<-cbind(QCmetrics, tissue.predictor$Predicted_Tissue)
#ggplot(data = QCmetrics, aes(x=Organ, fill= QCmetrics$Predicted_Tissue))+
...
setwd(params$PathToProjectFolder)
if("blood" %in% tolower(SampleSheet$Organ)){
  if(file.exists(file=  "Cellcounts.rdat")){
    print("Cell Counts exists and merge data together")
    ...
    print("CellCount created and saved as CellCounts.rdat saved")
  }
...
if("brain" %in% tolower(SampleSheet$Organ)){
  if(file.exists(file=  "Cellcounts.rdat")){
    print("Cell Counts exists and merge data together")
    load("CellCounts.rdat")
  } else {
    counts <- estimateCellCounts(RGset[,!SamplesFail], compositeCellType ="DLPFC", cellTypes = c( "NeuN_neg","NeuN_pos"))
    save(counts, file="CellCounts.rdat")
    print("CellCount created and saved as CellCounts.rdat saved")
  }
...
if(print(identical(rownames(SampleSheet), colnames(betas)))==TRUE){
  ...
  #save(msetEPIC.dasen, file =paste0('mset_dasen',params$Normalised))
